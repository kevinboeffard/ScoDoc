 ...
 * Displays the sessions of a selected day, a daily summary and a clickable month calendar.
 * @author Kevin Boeffard
 */
public class EmploiDuTempsEtudiantController {
    ...
    /**
     * Returns the sessions of the student's group for the given day, sorted by start time.
     *
    ...
    private List<Seance> recupererSeancesJour(LocalDate date) {
        ...
        for(Seance seance : App.listSeances) {
            if(seance.getGroupe().equals(etudiant.getGroupe()) && convertirDate(seance.getDate()).equals(date)) {
                seances.add(seance);
            }
        }
    ...
    private VBox creerCarteSeance(Seance seance) {
        ...
        Label details = new Label("Groupe " + seance.getGroupe().getNom() + "  ·  " + formatEnseignants(seance.getModule().getResponsables()));
    ...
    /**
     * Formats the list of teachers responsible for a module as "Prénom Nom, Prénom Nom".
     *
     * @param enseignants the list of teachers
     * @return the formatted list, or "Aucun enseignant" if empty
     */
    private String formatEnseignants(ArrayList<Enseignant> enseignants) {
        String texte = "";

        for(int i = 0; i < enseignants.size(); i++) {
            Enseignant enseignant = enseignants.get(i);
            texte += enseignant.getPrenom() + " " + enseignant.getNom();
            if(i < enseignants.size() - 1) texte += ", ";
        }

        return texte.isEmpty() ? "Aucun enseignant" : texte;
    }
    ...
    /**
     * Rebuilds the month calendar for the month of the displayed day.
     */
